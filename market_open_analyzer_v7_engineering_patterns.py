...
st.set_page_config(page_title="Paper Trading Lab V7 Engineering Patterns", page_icon="🧪", layout="wide")
...
RULES_FILE = DATA_DIR / "rules_v7_engineering.json"
ACCOUNT_FILE = DATA_DIR / "account_v3.json"
PENDING_FILE = DATA_DIR / "pending_signals_v7.csv"
ALERTS_FILE = DATA_DIR / "alerts_v5_8.csv"
ALERT_SETTINGS_FILE = DATA_DIR / "alert_settings_v5_8.json"
ENGINEERING_FILE = DATA_DIR / "engineering_predictions_v7.csv"
...
DEFAULT_RULES = {
    ...
    "engineering_require_when_ready": True,
    ...
    "engineering_min_samples": 10,
    "engineering_min_confidence": 0.58,
    "engineering_strong_confidence": 0.70,
    "engineering_min_similarity": 0.30,
    "engineering_min_direction_probability": 0.60,
    "engineering_min_expectancy_r": 0.25,
    "engineering_min_expectancy_gap_r": 0.35,
...
ENGINEERING_COLUMNS = [
    ...
    "sample_count", "mean_similarity", "long_probability", "short_probability",
...
def log_engineering_prediction(ticker, mode, bar_time, technical_side, result,
    ...
    try:
        ...
        row = {
            ...
            "long_probability": safe_float(result.get("long_probability"), 0.5),
            "short_probability": safe_float(result.get("short_probability"), 0.5),
...
def _engineering_channels(window_df):
    ...
    try:
        ...
        curvature = float(np.polyfit(x, price_atr, 2)[0][0])
    except Exception:
...
def _engineering_state(ch):
    ...
    if compression < 0.72 and abs(slope) < 0.12:
        return "COMPRESSION"
    if turns >= max(5, len(ch["price"]) // 4):
        return "OSCILLATION"
    if slope > 0.10 and recent_accel > 0.05:
        return "UP_ACCELERATION"
    if slope < -0.10 and recent_accel < -0.05:
        return "DOWN_ACCELERATION"
    if slope > 0.08 and recent_accel < -0.06:
        return "UP_WEAKENING"
    if slope < -0.08 and recent_accel > 0.06:
        return "DOWN_WEAKENING"
    if slope > 0.06 or recent_velocity > 0.15:
        return "UP_TREND"
    if slope < -0.06 or recent_velocity < -0.15:
        return "DOWN_TREND"
    return "NEUTRAL"
...
def engineering_pattern_analysis(full_df, mode, current_end=None, stop_r=1.0, target_r=1.5):
    ...
    min_similarity = float(rules.get("engineering_min_similarity", 0.30))
    ...
    mean_similarity = float(np.sum(weights * np.asarray([m["similarity"] for m in matches])))
    long_probability = float(np.sum(weights * (future_r > 0).astype(float)))
    short_probability = float(np.sum(weights * (future_r < 0).astype(float)))
...
def make_signal_from_history(ticker, mode, hist_df, market_hist_df=None):
    ...
    stop_r = abs(entry - safe_float(provisional_plan.get("stop"), entry)) / atr
    target_r = abs(safe_float(provisional_plan.get("target"), entry) - entry) / atr
    engineering = engineering_pattern_analysis(hist_df, mode, current_end=d.index[-1], stop_r=stop_r, target_r=target_r)
    predicted_side = str(engineering.get("predicted_side", "WAIT"))
    confidence = safe_float(engineering.get("confidence"), 0.0)
    ready = bool(engineering.get("ready", False))
    min_conf = float(rules.get("engineering_min_confidence", 0.58))
...
st.markdown(
    """
<div class="title-box">
<h1>🧪 Paper Trading Lab V7 Engineering Patterns</h1>
<p>מנוע חיזוי הנדסי: התאמת צורות מחיר, מהירות, תאוצה, ווליום ו־VWAP לתבניות עבר, בשילוב 5 דקות, QQQ וניהול סיכון.</p>
</div>
""",
    unsafe_allow_html=True,
)

tab_paper, tab_engineering, tab_ticket, tab_costs, tab_units, tab_rules, tab_account, tab_alerts, tab_backtest, tab_help = st.tabs([
    "🧪 Paper Trading",
    "🧠 מנוע תבניות",
    "🧾 Order Ticket Helper",
    "💸 עלויות",
...
with tab_engineering:
    st.subheader("🧠 מנוע תבניות הנדסי")
    st.caption("המנוע משווה את צורת המחיר, המהירות, התאוצה, הווליום, הטווח והמרחק מ־VWAP לתבניות קודמות. בדמו בלבד.")
    predictions = load_engineering_predictions()
    if predictions.empty:
        st.info("עדיין לא נשמרו תחזיות. הפעל סריקה בזמן שוק פתוח.")
    else:
        p = predictions.copy()
        for col in ["confidence", "mean_similarity", "long_probability", "short_probability", "long_target_rate", "short_target_rate"]:
            p[col] = pd.to_numeric(p[col], errors="coerce").fillna(0.0)
        latest = p.tail(300).copy()
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("תחזיות שנשמרו", f"{len(p):,}")
        c2.metric("ביטחון ממוצע", f"{latest['confidence'].mean()*100:.1f}%")
        confirmed = latest[latest["decision"].astype(str).str.contains("OPEN|CONFIRM", regex=True, na=False)]
        c3.metric("מועמדות מאושרות", f"{len(confirmed):,}")
        c4.metric("דמיון ממוצע", f"{latest['mean_similarity'].mean()*100:.1f}%")

        show = latest.sort_values("created_at", ascending=False).head(100).copy()
        display = pd.DataFrame({
            "זמן נר": show["bar_time"],
            "מניה": show["ticker"],
            "סוג": show["mode"],
            "טכני": show["technical_side"],
            "חיזוי הנדסי": show["predicted_side"],
            "החלטה": show["decision"],
            "ביטחון": show["confidence"].map(lambda x: f"{x*100:.0f}%"),
            "דוגמאות": pd.to_numeric(show["sample_count"], errors="coerce").fillna(0).astype(int),
            "דמיון": show["mean_similarity"].map(lambda x: f"{x*100:.0f}%"),
            "P לונג": show["long_probability"].map(lambda x: f"{x*100:.0f}%"),
            "P שורט": show["short_probability"].map(lambda x: f"{x*100:.0f}%"),
            "מצב": show["pattern_state"],
            "פונקציה": show["function_model"],
            "ציון": pd.to_numeric(show["final_score"], errors="coerce").fillna(0).astype(int),
        })
        st.dataframe(display, use_container_width=True, hide_index=True)

        selected_engine_ticker = st.selectbox("פירוט אחרון למניה", sorted(show["ticker"].astype(str).unique()))
        detail = show[show["ticker"].astype(str).eq(selected_engine_ticker)].head(1)
        if not detail.empty:
            st.info(str(detail.iloc[0]["reason"]))

    if st.button("🧹 נקה היסטוריית תחזיות הנדסיות", use_container_width=True):
        clear_engineering_predictions()
...
with tab_rules:
    ...
    with r2:
        ...
        engineering_min_samples = st.number_input("מינימום תבניות דומות לפני שהמנוע מחייב", 3, 100, int(rules.get("engineering_min_samples", 10)))
        engineering_top_k = st.number_input("מספר התבניות הדומות לחישוב", 5, 100, int(rules.get("engineering_top_k", 25)))
        engineering_min_conf = st.slider("ביטחון הנדסי מינימלי", 0.40, 0.90, float(rules.get("engineering_min_confidence", 0.58)), 0.01)
        engineering_strong_conf = st.slider("ביטחון חזק לשינוי כיוון", 0.50, 0.95, float(rules.get("engineering_strong_confidence", 0.70)), 0.01)
        engineering_window_half = st.number_input("אורך תבנית חצי שעה, נרות דקה", 15, 120, int(rules.get("engineering_window_half", 36)))
        engineering_horizon_half = st.number_input("אופק חיזוי חצי שעה, נרות", 5, 60, int(rules.get("engineering_horizon_half", 20)))
    ...
    engineering_enabled = st.checkbox("הפעל מנוע התאמת תבניות הנדסי", value=bool(rules.get("engineering_enabled", True)))
    engineering_require = st.checkbox("כאשר יש מספיק דוגמאות — חובה אישור הנדסי", value=bool(rules.get("engineering_require_when_ready", True)))
    engineering_override = st.checkbox("אפשר למנוע חזק להכריע מצב טכני גבולי", value=bool(rules.get("engineering_allow_strong_override", True)))
...
""",
    ...
    if st.button("💾 שמור חוקים", use_container_width=True):
        save_rules({
            ...
            "engineering_enabled": bool(engineering_enabled),
            "engineering_require_when_ready": bool(engineering_require),
            "engineering_allow_strong_override": bool(engineering_override),
            "engineering_min_samples": int(engineering_min_samples),
            "engineering_top_k": int(engineering_top_k),
            "engineering_min_confidence": float(engineering_min_conf),
            "engineering_strong_confidence": float(engineering_strong_conf),
            "engineering_window_half": int(engineering_window_half),
            "engineering_horizon_half": int(engineering_horizon_half),
